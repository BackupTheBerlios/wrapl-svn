...
#define PREFIX(SYMBOL)\
	if (Scanner->parse(tk ## SYMBOL)) {\
		expr_t *Function = new const_expr_t(Scanner->Token.LineNo, (Std$Object_t *)$ ## SYMBOL);\
...
static expr_t *parse_factor(scanner_t *Scanner) {
	...
	if (Scanner->parse(tkSTRBLOCK)) return new invoke_expr_t(Scanner->Token.LineNo,
		new const_expr_t(Scanner->Token.LineNo, "Wrapl$Runtime$CreateStringBlock"),
	...
	if (Scanner->parse(tkLBRACKET)) {
		...
		expr_t *Expr = new invoke_expr_t(LineNo,
			new const_expr_t(LineNo, "Std$List$Make"),
	...
	if (Scanner->parse(tkLBRACE)) {
		uint32_t LineNo = Scanner->Token.LineNo;
		expr_t *Expr = new invoke_expr_t(LineNo,
			new const_expr_t(LineNo, "Std$Table$Make"),
	...
	if (Scanner->parse(tkLESS)) {
		...
		if (Scanner->parse(tkLBRACKET)) {
			...
			//	Fields = new const_expr_t(Scanner->Tokegn.LineNo, Std$Array$new(0));
			//} else {
				Fields = new const_expr_t(Scanner->Token.LineNo, (Std$Object_t *)accept_fields(Scanner));
...
start:
	if (Scanner->parse(tkAT)) {
		expr_t *Function = new const_expr_t(Scanner->Token.LineNo, (Std$Object_t *)$AT);
	...
	if (Scanner->parse(tkLBRACKET)) {
		Expr->Next = accept_expr_list(Scanner);
		Expr = new invoke_expr_t(Scanner->Token.LineNo, new const_expr_t(Scanner->Token.LineNo, (Std$Object_t *)$INDEX), Expr);
		Scanner->accept(tkRBRACKET);
		goto start;
	};
	return Expr;
...
#define INFIX(SYMBOL, PRECEDENCE)\
	if (Scanner->parse(tk ## SYMBOL)) {\
		expr_t *Symbol = new const_expr_t(Scanner->Token.LineNo, (Std$Object_t *)$ ## SYMBOL);\
...
static expr_t *parse_expr(scanner_t *Scanner) {
	...
	if (Scanner->parse(tkTHEN)) {
		...
		if (Scanner->parse(tkELSE)) return new cond_expr_t(Expr->LineNo, Expr, Expr2, accept_expr(Scanner));
		return new cond_expr_t(Expr->LineNo, Expr, Expr2, 0);
	};
