...
#define emit(FORMAT, ARGS...) IO$Stream$writef(Output, FORMAT "\n", ## ARGS)
#define cemit(FORMAT, ARGS...) IO$Stream$writef(Compiler->Output, FORMAT "\n", ## ARGS)
...
operand_t *compiler_t::lookup(int LineNo, const char *Name) {DEBUG
			...
			case scope_t::SC_GLOBAL: {
				if (Operand->Type == operand_t::FUTR) {
					if (Operand->Import) {
						...
						asprintf(&Operand->Value, "%s$%s", Name, Operand->Import);
						Operand->Simple = 0;
					} else {
						Operand->Type = operand_t::CNST;
						Operand->Value = Name;
	...
	if (Riva$Config$get("Wrapl.AllowUndeclared")) {
		...
		case scope_t::SC_GLOBAL: {
			...
			emit("datasect");
			emit("%s: dd Std$Object$Nil", Operand->Address);
...
//	printf("\n");
//#endif
	operand_t *Closure = Start0->assemble(Frame, Compiler->Output, Constant);
...
operand_t *func_expr_t::precompile(compiler_t *Compiler, precomp_t &Type) {DEBUG
	...
	asprintf(&Constant->Value, "..@F%x", Constant);
	Constant->Simple = 0;
	Type = PC_PARTIAL;
	return Constant;
};
...
operand_t *qualident_expr_t::compile(compiler_t *Compiler, label_t *Start, label_t *Success) {DEBUG
	...
	for (qualident_expr_t::name_t *Name = Names->Next; Name; Name = Name->Next) {
		...
		const char *Module = Operand->Value;
		Operand = new operand_t;
		Operand->Type = operand_t::CNST;
		asprintf(&Operand->Value, "%s$%s", Module, Name->Ident);
		Operand->Simple = 0;
		cemit("extern %s", Operand->Value);
		Ident = Name->Ident;
	};
	Start->link(LineNo, Success);
	return Operand;
...
operand_t *const_expr_t::compile(compiler_t *Compiler, label_t *Start, label_t *Success) {DEBUG
	Start->link(LineNo, Success);
	return constant(Compiler);
};

operand_t *const_expr_t::constant(compiler_t *Compiler) {
	if (Operand == 0) {
		if (Value->Type == Std$Integer$SmallT) {
			Std$Integer_smallt *Integer = (Std$Integer_smallt *)Value;
			Operand = (operand_t *)integertable_get(Compiler->IntegerTable, Integer->Value);
			if (Operand == (operand_t *)0xFFFFFFFF) {
				Operand = new(operand_t);
				Operand->Type = operand_t::CNST;
				asprintf(&Operand->Value, "..@C%x", Operand);
				Operand->Simple = Value;
				cemit("\n\n;line %d", LineNo);
				cemit("datasect");
				cemit("%s:", Operand->Value);
				cemit("\tdd Std$Integer$SmallT");
				cemit("\tdd %d", Integer->Value);
				integertable_put(Compiler->IntegerTable, Integer->Value, Operand);
			};
		} else if (Value->Type == Std$String$T) {
			Std$String_t *String = (Std$String_t *)Value;
			Operand = new(operand_t);
			Operand->Type = operand_t::CNST;
			asprintf(&Operand->Value, "..@C%x", Operand);
			Operand->Simple = Value;
			...
			cemit("datasect");
			cemit("%s:", Operand->Value);			
			cemit("\tdd Std$String$T");
			cemit("\tdd Std$Integer$SmallT, %d", String->Length.Value);
			cemit("\tdd %d", String->Count);
			for (int I = 0; I < String->Count; ++I) {
				cemit("\tdd Std$Integer$SmallT, %d", String->Blocks[I].Length.Value);
				cemit("\tdd Std$Address$T, ..@S%x_%d", Operand, I);
			};
			cemit("\tdd 0, 0, 0, 0");
			for (int I = 0; I < String->Count; ++I) {
				Std$String_block *Block = String->Blocks + I;
				cemit("..@S%x_%d:", Operand, I);
				for (int J = 0; J < Block->Length.Value; ++J) {
					cemit("\tdb %d", ((char *)Block->Chars.Value)[J]);
				};
			};
		} else if (Value->Type == Std$Symbol$T) {
			Std$Symbol_t *Symbol = (Std$Symbol_t *)Value;
			Operand = (operand_t *)integertable_get(Compiler->SymbolTable, (uint32_t)Symbol);
			if (Operand == (operand_t *)0xFFFFFFFF) {
				Operand = new(operand_t);
				Operand->Type = operand_t::CNST;
				asprintf(&Operand->Value, "..@C%x", Operand);
				Operand->Simple = Value;
				cemit("\n\n;line %d", LineNo);
				cemit("section .symbols");
				cemit("%s: db \"%s\", 0", Operand->Value, Symbol->Name->Blocks->Chars.Value);
				integertable_put(Compiler->SymbolTable, (uint32_t)Symbol, Operand);
			};
		} else {
			printf("Type = %x\n", Value->Type);
			printf("const_expr_t::compile NOT DONE YET!\n");
		};
	};
	return Operand;
};
...
operand_t *when_expr_t::compile(compiler_t *Compiler, label_t *Start, label_t *Success) {DEBUG
	...
	Std$Object_t *Value = Operand->Simple;
	if (Value && (Value->Type == Std$Integer$SmallT)) {
		...
		if (Range->Max) {
			...
			Value = Operand->Simple;
			if (Value && (Value->Type != Std$Integer$SmallT)) Compiler->raise_error(Range->Max->LineNo, "Error: case is not correct type");
		...
		while (Range = Range->Next) {
			...
			if (Operand == 0) Compiler->raise_error(Range->Min->LineNo, "Error: case is not constant");
			Value = Operand->Simple;
			if (Value && (Value->Type != Std$Integer$SmallT)) Compiler->raise_error(Range->Min->LineNo, "Error: case is not correct type");
		...
		return Register;
	} else if (Value && (Value->Type == Std$String$T)) {
		...
		while (Range = Range->Next) {
			...
			if (Operand == 0) Compiler->raise_error(Range->Min->LineNo, "Error: case is not constant");
			Value = Operand->Simple;
			if (Value && (Value->Type != Std$String$T)) Compiler->raise_error(Range->Min->LineNo, "Error: case is not correct type");
	...
	for (block_expr_t::localdef_t *Def = Defs; Def; Def = Def->Next) {
		...
		case PC_NONE: {
			...
			label_t *Failure = new label_t;
				...
				Compiler->pop_expression();
			frame_t *Frame = Compiler->pop_function();
			operand_t *Closure = Start->assemble(Frame, Compiler->Output);
			operand_t *Operand = new operand_t;
			Operand->Type = operand_t::CNST;
			asprintf(&Operand->Value, "..@Z%x", Operand);
			Operand->Simple = 0;
			cemit("\n\n;line %d", LineNo);
			cemit("section .constants");
			cemit("%s: dd ..@ZI%x", Operand->Value, Operand);
			cemit("textsect");
			cemit("..@ZI%x:", Operand);
			cemit("\tmov ecx, %s", Closure->Value);
			cemit("\tmov eax, [value(ecx).Type]");
			cemit("\tcall [type(eax).Invoke]");
			// check if eax is SUCCESS/SUSPEND
			cemit("\tmov eax, ecx");
			cemit("\tret");
...
operand_t *module_expr_t::compile(compiler_t *Compiler, label_t *Start, label_t *Success) {DEBUG
	...
	operand_t *Operand = new operand_t;
	Operand->Type = operand_t::CNST;
	Operand->Value = "HELLO"; //(Std$Object_t *)Module;
	Operand->Simple = 0;
	Compiler->declare(Name, Operand);
	IO$Stream$writes(Compiler->Output, Prologue);
	cemit("datasect");
	...
	if (Body) {
		...
		frame_t *Frame = Compiler->pop_function();
		operand_t *Closure = Start->assemble(Frame, Compiler->Output);
		cemit("global %s$INIT", Name);
		cemit("%s$INIT equ %s", Name, Closure->Value);
	};
	Start->link(LineNo, Success);
	return Operand;
};
