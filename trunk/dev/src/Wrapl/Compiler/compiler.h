...
struct compiler_t {
	...
	IO$Stream_t *Output;
	integertable_t IntegerTable, SymbolTable;

	compiler_t(IO$Stream_t *Output) {
...
struct const_expr_t : expr_t {
	Std$Object_t *Value;
	operand_t *Operand;
	const_expr_t(int LineNo, Std$Object_t *Value) {
		this->LineNo = LineNo;
		this->Value = Value;
		this->Operand = 0;
	};
	const_expr_t(int LineNo, const char *Value) {
		this->LineNo = LineNo;
